...
class SnakeQAgent():
    ...
    def get_action(self, state):
        ...
        #self.table[state] retrieves row which contains the Q-values for all possible actions available in that state.
        return np.argmax(self.table[state]) #This function finds the index of the action that has the highest Q-value in that row.
    ...
    def train(self):
        for i in range(1, self.num_episodes + 1):
            ...
            # print updates (every 25 episodes)
            if i % 25 == 0:
            ...
            if (i < 500 and i % 10 == 0) or (i >= 500 and i < 1000 and i % 100 == 0) or (i >= 1000 and i % 500 == 0):
            ...
            while not done:
                ...
                # Check if the snake's length has changed after taking the action
                if length != self.env.snake_length:
                    length = self.env.snake_length
                    steps_without_food = 0
                    
                # If the snake has not eaten food for 1000 steps, break out of the loop    
